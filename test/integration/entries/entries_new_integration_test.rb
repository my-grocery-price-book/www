...
class EntriesNewIntegrationTest < EntriesIntegrationTest
  setup do
  ...
  context 'GET /books/:book_id/pages/:page_id/entries/new' do
    context 'book region codes sets' do
      ...
      context 'page exists' do
        should 'render the new form ' do
          get "/books/#{price_book.to_param}/pages/#{book_page.to_param}/entries/new"
        ...
        should 'set book_store_create_return session' do
          get "/books/#{price_book.to_param}/pages/#{book_page.to_param}/entries/new"
          assert_equal new_book_page_entry_path(price_book, book_page), session[:book_store_create_return]
        end
      end
    ...
    context 'no book region codes set' do
      ...
      should 'set a book_regions_create_return' do
        get "/books/#{price_book.to_param}/pages/#{book_page.to_param}/entries/new"
        assert_equal(new_book_page_entry_path(price_book, book_page), session[:book_regions_create_return])
      end
    end
  end
end
