...
class Purchases::SendItemToApiJob < ActiveJob::Base
  ...
  def perform(api_root:, api_region_code:, api_key:, date_on:, store:, location:, item:)
    ...
    connection.post "/#{api_region_code}/entries", api_key: api_key, product_brand_name: item.product_brand_name,
                                                   generic_name: item.regular_name, category: item.category,
                                                   package_size: item.package_size, package_unit: item.package_unit,
                                                   quantity: item.quantity, total_price: item.total_price,
                                                   date_on: date_on, store: store, location: location
  end
