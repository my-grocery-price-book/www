...
class ShopperApiKey < ActiveRecord::Base
  def self.api_key(shopper_id:, api_root:, shopper_email:)
    shooper_api_key = find_by(shopper_id: shopper_id, api_root: api_root)
    return shooper_api_key.api_key if shooper_api_key
    connection = create_faraday_connection(api_root)
    response = connection.post '/users', email: shopper_email
    response_hash = JSON.parse(response.body)
    create!(shopper_id: shopper_id, api_root: api_root, api_key: response_hash['api_key'])
