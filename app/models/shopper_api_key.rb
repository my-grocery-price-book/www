class ShopperApiKey < ActiveRecord::Base
  def self.api_key(shopper:, api_url:)
    shooper_api_key = find_by(shopper_id: shopper.id, api_url: api_url)
    ...
    create!(shopper_id: shopper.id, api_url: api_url, api_key: response_hash['api_key'])
